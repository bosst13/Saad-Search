...
class PropertyController extends Controller
{
    ...
    public function search()
    {
        ...
        $description = Description::where('title', 'LIKE', '%',$search_text.'%')->with('title')->get();

        return view('description.search', compact('description'));
    }
    */
    /*
    public function search()
    {
       ...
       if ($search){   
        $posts = Description::where('title', 'LIKE', "%{$search}%")->simplePaginate(3);      
       }
       else{
        $posts = Description::simplePaginate(3);
       }
       return view('showproperty')
       ->with('title', Description::all());      
    }
    */
...
//{
  ...
  //  $search = $request->search;
    /*if ($search) {   
        $properties = Description::where('title', 'LIKE', "%{$search}%")->simplePaginate(3);     
    } else {
        $properties = Description::simplePaginate(3);
    }
    dd($properties);
    return view('showproperty', ['title' => $properties]);
    //return view('showproperty')->with('title', $posts);
    */

    
    /*  public function searchproperty()
    {
        //$properties = request()->query('search');
        ...
        //$properties = Property::with(['owner.account'])
            ...
            if($properties){
                $properties = Description::where('title', 'LIKE', "%{$properties}%")->simplePaginate(3);
            } else {
                $properties = Description::simplePaginate(3);           
            }
        ...
        return view('property.searchproperty', compact('properties'));
        //return view('showproperty', compact('properties'));
    } */
    ...
    public function searchproperty()
{
    ...
    // Retrieve properties based on the search query
    ...
    $properties = $properties->simplePaginate(3);
